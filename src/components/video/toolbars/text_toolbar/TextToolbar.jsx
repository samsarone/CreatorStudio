...
export default function TextToolbar(props) {
  ...
  useEffect(() => {
    ...
    if (activeItemList) {
      ...
      if (currentItem) {
        setXValue(currentItem.config.x);
        setYValue(currentItem.config.y);
        setWidthValue(currentItem.config.width);
        setHeightValue(currentItem.config.height);
      }
    }
